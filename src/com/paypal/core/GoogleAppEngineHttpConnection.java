...
public class GoogleAppEngineHttpConnection extends HttpConnection {
	...
	public void setupClientSSL(String certPath, String certKey, boolean trustAll)
		...
		if (trustAll) {
			LoggingManager
					.warn(GoogleAppEngineHttpConnection.class,
							"The PayPal SDK cannot be configured to trust all certificates when used on Google App Engine. The 'http.TrustAllConnection' config property will be ignored");
		}
	}
	...
	public void createAndconfigureHttpConnection(
		...
		// As of GAE 1.6.5, SSL behaviour can't really be configured, therefore
		// we cannot
		// use SSLUtil to install the custom trust manager that relaxes
		// verification of
		// server certificates.
